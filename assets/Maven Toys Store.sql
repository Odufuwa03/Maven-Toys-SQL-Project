...
SELECT
	((SUM(p.product_price * i.stock_on_hand))) / (select 
	sum(p.product_price * s.units) / count(distinct s.date) as avg_daily_sales
from 
	products p
join 
	sales s
on
	s.product_id = p.product_id) as duration_to_last
...
/*
Are sales been lost with the product out_of_shock
*/
Select
	distinct s.store_location
from
	inventory i
join 
	stores s
...
where
	stock_on_hand =0
and i.product_id in (select product_id from sales)
